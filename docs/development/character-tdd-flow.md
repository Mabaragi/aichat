...
이 문서는 `Character` 기능을 MVP 수준으로 구현할 때 따를 TDD 작업 순서를 정리한다. 기준 명세는 루트의 `ai_debate_platform_mvp_spec.md`이고, 도메인 규칙 요약은 [Character 도메인 개발 흐름](./character-domain-flow.md)을 함께 참고한다.

...
@Service
public class StartDebateSessionUseCase {

    private final DebateSessionRepository debateSessionRepository;
    private final TimeProvider timeProvider;

    public StartDebateSessionUseCase(DebateSessionRepository debateSessionRepository,
    ...
    @Transactional
    public DebateSessionLifecycleView execute(StartDebateSessionCommand command) {
        DebateSession session = requireSession(command.actor(), command.sessionId());
        if (session.getStatus() != DebateSessionStatus.CREATED) {
            throw new BusinessException(
                    ErrorCode.INVALID_SESSION_STATE,
                    "session must be CREATED to start"
            );
        }

        session.start(timeProvider.now());

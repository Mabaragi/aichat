    ...
    void exposesImplementedApiDocumentationWithBearerSecurityAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/debate-sessions/{sessionId}/turns/generate'].post")
                        .exists())
                .andExpect(jsonPath("$.paths['/api/public/categories'].get").exists())
                .andExpect(jsonPath("$.paths['/api/public/characters'].get").exists())
                .andExpect(jsonPath("$.paths['/api/public/characters/{characterId}'].get").exists())
                .andExpect(jsonPath("$.paths['/api/public/debate-sessions'].get").exists())
                .andExpect(jsonPath("$.paths['/api/public/debate-sessions/{sessionId}'].get").exists())
                .andExpect(jsonPath("$.paths['/api/public/debate-sessions/{sessionId}/turns'].get")
                        .exists())
                .andExpect(jsonPath("$.paths['/api/shared-contents/{slug}']").doesNotExist())
                .andExpect(jsonPath("$.components.schemas.ErrorResponse").exists())
                .andExpect(jsonPath("$.components.schemas.CategorySummaryResponse").exists())
                .andExpect(jsonPath("$.components.schemas.PublicCharacterPageResponse").exists())
                .andExpect(jsonPath("$.components.schemas.PublicDebateSessionPageResponse").exists())

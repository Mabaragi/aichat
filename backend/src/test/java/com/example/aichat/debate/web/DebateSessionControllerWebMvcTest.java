...
class DebateSessionControllerWebMvcTest {
    ...
    void setUp() {
        ...
        DebateSessionController controller = new DebateSessionController(
                ...
                startDebateSessionUseCase,
                completeDebateSessionUseCase,
                objectMapper
        );
    ...
    @Test
    void startSessionDelegatesAndReturnsStatusSummary() throws Exception {
        when(startDebateSessionUseCase.execute(any())).thenReturn(new DebateSessionLifecycleView(
                1L,
                DebateSessionStatus.RUNNING,
                LocalDateTime.of(2026, 6, 10, 12, 1),
                null
        ));

        mockMvc.perform(post("/api/debate-sessions/1/start")
                        .principal(authentication())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.status").value("RUNNING"))
                .andExpect(jsonPath("$.startedAt").value("2026-06-10T12:01:00"));

        ArgumentCaptor<StartDebateSessionCommand> captor =
                ArgumentCaptor.forClass(StartDebateSessionCommand.class);
        verify(startDebateSessionUseCase).execute(captor.capture());
        assertThat(captor.getValue().sessionId()).isEqualTo(1L);
        assertThat(captor.getValue().actor().userId()).isEqualTo(1L);
    }

    @Test
    void completeSessionDelegatesAndReturnsStatusSummary() throws Exception {
        when(completeDebateSessionUseCase.execute(any())).thenReturn(new DebateSessionLifecycleView(
                1L,
                DebateSessionStatus.COMPLETED,
                LocalDateTime.of(2026, 6, 10, 12, 1),
                LocalDateTime.of(2026, 6, 10, 12, 10)
        ));

        mockMvc.perform(post("/api/debate-sessions/1/complete")
                        .principal(authentication())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.status").value("COMPLETED"))
                .andExpect(jsonPath("$.endedAt").value("2026-06-10T12:10:00"));

        ArgumentCaptor<CompleteDebateSessionCommand> captor =
                ArgumentCaptor.forClass(CompleteDebateSessionCommand.class);
        verify(completeDebateSessionUseCase).execute(captor.capture());
        assertThat(captor.getValue().sessionId()).isEqualTo(1L);
        assertThat(captor.getValue().actor().userId()).isEqualTo(1L);
    }

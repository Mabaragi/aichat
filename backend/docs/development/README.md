...
이 디렉터리는 사람 개발자가 backend 기능을 구현할 때 참고하는 개발 흐름 문서를 둔다. 저장소의 실제 기준 명세는 `../ai_debate_platform_mvp_spec.md`이며, 이 문서는 명세를 구현 순서로 풀어 쓰는 보조 문서다.

## 문서 목록

- [Backend TDD Guideline](./backend-tdd-guideline.md): 백엔드 기능, 도메인 규칙, 버그 수정 작업에서 TDD를 우선 적용하는 기준과 계층별 테스트 우선순위.
...
- 공통 AWS 배포 절차는 저장소 루트 `docs/deployment/aws-cicd.md`에서 관리한다.

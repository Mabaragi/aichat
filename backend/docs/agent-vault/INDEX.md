...
이 문서는 backend 작업에 필요한 컨텍스트만 골라 읽기 위한 라우터다. 저장소 루트 `../AGENTS.md`와 backend `AGENTS.md`를 읽은 뒤 여기서 관련 문서만 추가로 연다.

## Project Guides

- `guides/project-map.md`: 프로젝트 목적, 기술 스택, 주요 패키지 경계, 현재 구현 상태를 요약한다. 새 기능을 추가하거나 낯선 도메인을 건드릴 때 읽는다.
- `guides/character-cli.md`: `aichat` CLI 명령 계약, 입력/출력, exit code, 예제를 정리한다. `character`, `user`, `debate` CLI를 직접 호출하는 에이전트 작업 전에 읽는다.
